...
import com.cgzz.mapbox.jturf.callback.FlattenEachCallback;
...
public final class FlattenEachHelper {
    ...
    public static void flattenEach(Geometry geometry, FlattenEachCallback callback) {
        GeomEachHelper.geomEach(geometry, (g, featureIndex, properties, id) -> {
            ...
            if (g == null) {
                return callback.accept(null, featureIndex, 0);
            }
            ...
            switch (type) {
                ...
                case LINE_STRING:
                case POLYGON:
                    return callback.accept(Feature.fromGeometry(g, properties, id), featureIndex, 0);
            }
            ...
            switch (type) {
                case MULTI_POINT: {
                    ...
                    for (int i = 0, size = coordinates.size(); i < size; i++) {
                        if (!callback.accept(Feature.fromGeometry(coordinates.get(i), properties, id), featureIndex, i)) {
                            return false;
                        }
                    }
                    break;
                ...
                case MULTI_LINE_STRING: {
                    ...
                    for (int i = 0, size = coordinates.size(); i < size; i++) {
                        if (!callback.accept(Feature.fromGeometry(LineString.fromLngLats(coordinates.get(i)), properties, id), featureIndex, i)) {
                            return false;
                        }
                    }
                    break;
                ...
                case MULTI_POLYGON: {
                    ...
                    for (int i = 0, size = coordinates.size(); i < size; i++) {
                        if (!callback.accept(Feature.fromGeometry(Polygon.fromLngLats(coordinates.get(i)), properties, id), featureIndex, i)) {
                            return false;
                        }
                    }
                    break;
